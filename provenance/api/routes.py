...
def commit(repo, add, branch='master', message='Auto commit'):
  ...
  if add in repo.untracked_files:
    repo.git.add(add);
    if has_changed is False: has_changed = True;
  ...
  if has_changed is True: repo.git.commit('-m', message);
...
async def add(request):
  ...
  if notebook and notebook["notebook"] and "name" in notebook["notebook"]: # is save
    ...
    with orm.db_session: variable_values = variables.extract_variable_values(repo, db.Expression.select());
