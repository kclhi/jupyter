...
def extract_variable_values(repo, expressions):
  ...
  for file in repository.untracked_files: repository.git.add(file, N=True);
        ...
        for expression in expressions:
          ...
          if match:
            ...
            for variable in expression.template_variable:
              ...
              if variable.name in [variable_name["name"] for variable_name in last_substitution]: last_substitution=[];
              last_substitution.append(substitution_variable);
              # Associate new substitution (zone) with template.
              if len(last_substitution)==1: template_substitutions.setdefault(variable.template.name,[]).append(last_substitution);
